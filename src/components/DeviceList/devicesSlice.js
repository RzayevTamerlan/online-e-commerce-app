  ...
  async (_, { getState }) => {
    ...
    if (brandFilter && typeFilter) {
      return await request(
        `http://localhost:3000/products?brand=${brandFilter}&type=${typeFilter}&_page=${page}&_limit=16`
      );
    } else if (brandFilter) {
      return await request(
        `http://localhost:3000/products?brand=${brandFilter}&_page=${page}&_limit=16`
      );
    } else if (typeFilter) {
      return await request(
        `http://localhost:3000/products?&type=${typeFilter}&_page=${page}&_limit=16`
      );
    } else {
      return await request(
        `http://localhost:3000/products?&_page=${page}&_limit=16`
      );
    }
  }
);
  ...
  async (_, { getState }) => {
    ...
    if (brandFilter && typeFilter) {
      const req = await request(
        `http://localhost:3000/products?brand=${brandFilter}&type=${typeFilter}`
      );
      return Object.keys(req).length;
    } else if (brandFilter) {
      const req = await request(
        `http://localhost:3000/products?brand=${brandFilter}`
      );
      return Object.keys(req).length;
    } else if (typeFilter) {
      const req = await request(
        `http://localhost:3000/products?&type=${typeFilter}`
      );
      return Object.keys(req).length;
    } else {
      const req = await request(`http://localhost:3000/products`);
      return Object.keys(req).length;
    }
  }
);
...
export const fetchDevice = createAsyncThunk(
  ...
  async (id) => {
    const { request } = useHttp();
    return await request(`http://localhost:3000/products/${id}`);
  }
);
